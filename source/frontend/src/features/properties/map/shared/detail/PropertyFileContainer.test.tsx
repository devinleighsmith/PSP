  ...
  it('loads from the expected sources', async () => {
    ...
    expect(mockAxios.history.get).toContainEqual(
      expect.objectContaining({
        url: 'https://delivery.apps.gov.bc.ca/ext/sgw/geo.bca?service=WFS&version=2.0.0&outputFormat=json&typeNames=geo.bca%3AWHSE_HUMAN_CULTURAL_ECONOMIC.BCA_FOLIO_LEGAL_DESCRIPTS_SV&srsName=EPSG%3A4326&request=GetFeature&cql_filter=PID_NUMBER+ilike+%27%25123456789%25%27',
      }),
    );
    expect(mockAxios.history.get).toContainEqual(
      expect.objectContaining({
