...
export const MotiInventoryContainer: React.FunctionComponent = () => {
   ...
   */
  const getParcelLayerResponse = async (latLng?: LatLngLiteral, pid?: string) => {
    if (!!pid) {
  ...
  const populateForm = async (
    ...
    try {
      ...
      const ltsaResponse = await handleLtsaRequest(getParcelInfo(properties?.PID), properties?.PID);
  ...
  const formikParcelDataPopulateCallback = (
    ...
    const newValues = {
      ...
      regionalDistrict: getIn(parcelLayerResponse, 'features.0.properties.REGIONAL_DISTRICT') ?? '',
      ...
      {!sidebarContext || sidebarContext === SidebarContextType.ADD_PROPERTY_TYPE_SELECTOR ? (
        ...
        <>
          ...
          <PropertySearchForm
            ...
            handleGeocoderChange={async (data: IGeocoderResponse, nameSpace?: string) => {
